...
#Embedded file name: /usr/lib/enigma2/python/magicstar/magicstar_addon_manager.py
...
import traceback
import StringIO
from magicstar.magicstar_tools import wyszukaj_re, unload_modules
from magicstar.magicstar_skins import EGAddonInfo_Skin, EG_InternetAddons_Skin, EGAddonRemove_Skin, AddonManager_Skin, EG_Manual_installation_Skin, EGAddonMenu_Skin
...
class EGAddonMenu(Screen):
    ...
    def KeyOk(self):
        ...
        if self.sel == 0:
            ...
            self.session.openWithCallback(self.runUpgrade, MessageBox, _('Do you want to update your magicstar image?') + '\n' + _('\nAfter pressing OK, please wait!'))
        elif self.sel == 5:
            if fileExists('/etc/user_addon.txt'):
                ...
                plik = 'There is no user_addon.txt file in /etc with server url you need to build one for your self sorry!'
    ...
    def updateList(self):
        ...
        name = _('Download magicstar Addons')
        desc = _('Download extensions from magicstar server...')
        idx = 0
        res = (name,
         png,
         idx,
        ...
        mypixmap = mypath + 'magic_icons/addon_manual.png'
        png = LoadPixmap(mypixmap)
        name = _('Manual Install magicstar and IPK packages')
        ...
        png = LoadPixmap(mypixmap)
        name = _('Remove magicstar Packages')
        desc = _('Remove installed magicstar packages...')
        idx = 2
        res = (name,
         png,
         idx,
        ...
        self.list.append(res)
        self['list'].list = self.list
# okay decompyling magicstar_addon_manager.pyo
