...
class magicstarWicarddMain(Screen):
	...
	def updateList(self):
		self.list = []
		mypath = "/usr/share/enigma2/skin_default/magic_icons"
	...
	def KeyOk(self):
		...
		if self.sel:
			...
			self.sel
		if self.sel == 0:
			self.session.open(magicstarWicarddInfo)
		else:
			if self.sel == 1:
				self.session.open(magicstarWicarddUs)
			else:
				if self.sel == 2:
					self.session.open(magicstarWicarddReader0)
				else:
					if self.sel == 3:
						self.session.open(magicstarWicarddReader1)
					else:
						if self.sel == 4:
							self.session.open(magicstarWicarddStat)



class magicstarWicarddReader0(Screen):
	...
	def __init__(self, session):
		...
		self['actions'] = ActionMap(['WizardActions'], {'ok': self.close, 'back': self.close})
...
class magicstarWicarddReader1(Screen):
	skin = '\n\t<screen position="center,center" size="640,450" title="Wicardd Main Info">\n\t\t<widget name="infotext" position="10,10" size="620,410" font="Regular;22" halign="center" />\n\t\t<widget name="Linconn" position="0,420" size="680,30" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" />\n\t</screen>'
	def __init__(self, session):
		Screen.__init__(self, session)
		self['infotext'] = Label()
...
class magicstarWicarddStat(Screen):
