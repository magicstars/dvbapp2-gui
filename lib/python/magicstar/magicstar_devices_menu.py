...
from magicstar.magicstar_tools import wyszukaj_re, runBackCmd
from magicstar.magicstar_skins import EGHardDriveInfo_Skin, EGDeviceManager_Skin, EGDeviceManager_Setup_Skin
...
class EGDeviceManager_Setup(Screen,
 ConfigListScreen):
    ...
    def buildMy_rec(self, device):
                ...
                if (d1 == '/media/magicstarboot'):
                        item = NoSave(ConfigSelection(default='/media/magicstarboot', choices=[('/media/magicstarboot', '/media/magicstarboot')]))
                if dtype == 'Linux':
                        dtype = 'ext3'
                else:
                        dtype = 'auto'
    ...
    def saveMypoints(self):
                ...
                message = _("Great ST7TEAM your Devices changes and need a system restart to take effects.\nRestart your Box now?")
	...
	system("unmount /mnt/card;umount /mnt/cf;umount /mnt/hdd;umount /mnt/usb;umount /mnt/usb2;umount /mnt/usb3")
        nobox = self.session.open(MessageBox, (_("magicstar Devices Manager has just unmounted all your devices.")), MessageBox.TYPE_INFO, 3)
