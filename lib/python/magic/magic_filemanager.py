...
from magic.magic_skins import EGFileManagerConfig_Skin, EGFileManager_Skin, EGFileViewer_Skin, EGPicViewer_Skin, EGFileManager_InfoMenu_Skin, \
...
class EGFileManagerConfig(ConfigListScreen,Screen):
    ...
    def onLayout(self):
        self.setTitle(_("magic File Manager - Settings"))
    ...
    def ok(self):
        if self.SOURCELIST.canDescent(): # isDir
            ...
            if self.SOURCELIST.getCurrentDirectory(): #??? when is it none
                self.setTitle("magic File Manager - " + self.SOURCELIST.getCurrentDirectory())
        else:
            self.onFileAction()
...
dmnapi_py = "python /usr/lib/enigma2/python/magic/magic_dmnapi.pyo"
...
### MENU
    def go(self):
		...
		os.system("chmod 755 /usr/lib/enigma2/python/magic/magic_dmnapi.pyo")
...
class EGConsoleView(Console):
	def __init__(self, session, os_commando):
		...
		Console.__init__(self, session, title = "magic File Manager - Console", cmdlist = [ os_commando ])
