...
from magic.magic_tools import runBackCmd, unload_modules, wyszukaj_re, checkkernel
...
from magic.magic_skins import magicBackupPanel_Skin, magicBackupPanel_Step2_Skin, magicBackupPanel_Step3_Skin, magicRestorePanel_Step1_Skin, magicRestorePanel_Step2_Skin

class magicBackupPanel(Screen):
	def __init__(self, session, args=None):
		self.skin = magicBackupPanel_Skin
		...
		self['key_green'] = Label(_('Restore magic'))
		self['key_yellow'] = Label(_('Backup magic'))
	...
	def updateT(self):
		self.mybackupfile = ''
		mytext = (_('There is not any magic Backup file on connected devices!'))
		mytext2 = (_(' '))
		myfile = ''
		...
		if fileExists('/etc/magic/.magicbackup_location'):
			...
			for fil in myscripts:
				if (fil.find('_magic_Backup.egi') != -1):
				    mytext = 'There is magic Backup file:'
				    mytext2 = 'Date:                      Device:                             Name:'
		else:
			fileExists('/etc/magic/.magicbackup_location')
		if myfile == '':
			myfile = self.scan_mediA()
		if fileExists('/etc/magic/.magicbackup_files'):
			fileExists('/etc/magic/.magicbackup_files')
			f = open('/etc/magic/.magicbackup_files', 'r')
			mypath = f.readline().strip()
			f.close()
	...
	def scan_mediA(self):
		out = open('/etc/magic/.magicbackup_files', 'w')
		backup = 'ok'
	...
	def restorE(self):
		...
		if m == 1:
			...
			if check == False:
			    ...
			    if backup_file != '':
				    message = (_('Do you really want to restore the magic Backup:\n ')) + self.mybackupfile + ' ?'
				    self.session.openWithCallback(self.restorE_2, MessageBox, message, MessageBox.TYPE_YESNO)
			    else:
				    system('umount /media/magicbackup_location')
				    system('rmdir /media/magicbackup_location')
				    self.session.open(MessageBox, _('Sorry, magic Backup not found.'), MessageBox.TYPE_INFO)
		else:
			self.session.open(MessageBox, _('Sorry: Wrong image in flash found. You have to install in flash magic Image'), MessageBox.TYPE_INFO, 3)
	...
	def restorE_2(self, answer):
		if answer is True:
			...
			self.session.open(magicRestorePanel_Step1, backup_file, backup_path)




class magicBackupPanel_Step2(Screen):
	def __init__(self, session):
		self.skin = magicBackupPanel_Step2_Skin
		Screen.__init__(self, session)
		self.list = []
		self["config"] = MenuList(self.list)
		self['key_green'] = Label(_('Backup magic'))
		self['key_red'] = Label(_('Cancel'))
	...
	def saveMysets(self):
		...
		out = open('/etc/magic/.magicbackup_location', 'w')
		out.write(mysel[1])
		out.close()
		if fileExists('/etc/magic/.magicbackup_location'):
			fileExists('/etc/magic/.magicbackup_location')
			self.session.openWithCallback(self.myclose, magicBackupPanel_Step3)
		else:
			fileExists('/etc/magic/.magicbackup_location')
...
class magicBackupPanel_Step3(Screen):
	def __init__(self, session):
	...
	def procesS(self):
		...
		if cur != 'END':
			...
			cmd = 'tar -cf magic_Backup.tar etc lib media usr scripts'
			rc = system(cmd)
			import datetime
			import time
			now = datetime.datetime.now()
			czas = now.strftime("%Y%m%d")
			filename = '../'+czas + '_magic_Backup.egi' 
			os_rename('magic_Backup.tar', filename)
			chdir(mydir)
			self.session.open(MessageBox, _("magic Backup complete! Please wait..."), MessageBox.TYPE_INFO, timeout=4)
			self.close()
			
			# /media/hdd/20110903_magicBackup.egi
...
class magicRestorePanel_Step1(Screen, ConfigListScreen):
	def __init__(self, session, mypath,backpath):
		self.skin = magicRestorePanel_Step1_Skin
		Screen.__init__(self, session)
		self.mypath = mypath
	...
	def Continue(self):
		...
		for x in self['config'].list:
			if x[1].value == True:
				...
				continue
		mylist.append('END')
		self.session.open(magicRestorePanel_Step2, self.mypath, mylist, self.backpath)
		self.close()



class magicRestorePanel_Step2(Screen):
	def __init__(self, session, mypath, mylist, myback):
		self.skin = magicRestorePanel_Step2_Skin
		Screen.__init__(self, session)
		self.mytext = 'Files extraction in progress...'
		self['status'] = MultiPixmap()
		self['status'].setPixmapNum(0)
		self['label'] = Label('')
		self.mypath = myback + '/magicbackup_location' # /media/hdd//magicbackup_location
		self.mybackupfile = mypath # /media/hdd/20110903_magicBackup.egi
		self.mylist = mylist
		self.count = 0
		self.go = False
	...
	def procesS(self):
		...
		if cur != 'END':
			...
			ret = system('umount /media/magicbackup_location')
			ret = system('rmdir /media/magicbackup_location')
...
class EGFullBackup(Screen, ConfigListScreen):
	...
	def updateList(self):
		...
		print len(self.list)
		if len(self.list) < 1:
			self['label2'].setText(_('Sorry no device found to store backup. Please check your media in magic devices panel.'))
			self.deviceok = False
	...
	def saveMysets(self):
		if self.deviceok == True:
			mysel = self['config'].getCurrent()
			mytitle = 'magic Full Backup on: ' + mysel[1]
			cmd = '/usr/bin/magic_backup.sh ' + mysel[1]
			self.session.open(Console, title=mytitle, cmdlist=[cmd], finishedCallback=self.myclose)
		else:
  			self.session.open(MessageBox, _('Sorry, there is not any connected devices in your STB.\nPlease connect HDD or USB to full backup Your magic Image!'), MessageBox.TYPE_INFO)
