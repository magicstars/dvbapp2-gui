...
from magic.magic_skins import *

class magicGreenPanel:
	def __init__(self):
		self["magicGreenPanel"] = ActionMap( [ "InfobarSubserviceSelectionActions" ],
			{
				"magicGreenPanelShow": (self.showmagicGreenPanel),
			})

	def showmagicGreenPanel(self):
...
class EGExtrasMenu(Screen):
	...
	def KeyOk(self):
		...
		self.sel = self.sel[2]
		if (self.sel == 0):
			from magic.magic_panel import EGSwapManager
			self.session.open(EGSwapManager)
		elif (self.sel == 1):
			from magic.magic_panel import EGNetBrowser
			...
			from magic.magic_services import EGServicesMenu
			self.session.open(EGServicesMenu)
		elif (self.sel == 5):
			from Screens.magic_wizard import magicWizardSetup
			self.session.open(magicWizardSetup)
		elif (self.sel == 6):
			from magic.magic_infobar_setup import EGInfoBarSetup
			self.session.open(EGInfoBarSetup)
		elif (self.sel == 7):
			from magic.magic_panel import DvbSnoop
			self.session.open(DvbSnoop)
		elif (self.sel == 8):
			from magic.magic_panel import EGProcessInfo
			self.session.open(EGProcessInfo)
		elif (self.sel == 9):
			from magic.magic_panel import EGKernelInfo
			self.session.open(EGKernelInfo)
		elif (self.sel == 10):
			from magic.magic_panel import EGEnigma2ConfigInfo
			...
			from magic.magic_panel import EGSetupHttpStream
...
class EGGreenPanel(Screen):
	...
	def Addons(self):
		m = checkkernel()
		if m == 1:
			from magic.magic_addon_manager import EGAddonMenu
	...
	def File(self):
		    m = checkkernel()
		    if m == 1:
			from magic.magic_filemanager import EGFileManager
	...
	def Script(self):
		m = checkkernel()
		if m == 1:
			from magic.magic_panel import SmartScript
