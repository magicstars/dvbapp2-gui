	...
	def run(self):
		if self.index == self.STATE_UPDATE:
			...
			if softcam_name == "cccam":
				cmd = (";rm /tmp/Addon.tgz;wget -q http://magic-image.com./image-feed/enigma2/softcam/cccam_230_magic_E2.tar.gz -O /tmp/Addon.tgz")
			elif softcam_name == "wicardd":
				cmd = (";rm /tmp/Addon.tgz;wget -q http://magic-image.com./image-feed/enigma2/softcam/wicardd_110_magic_E2.tar.gz -O /tmp/Addon.tgz")
			elif softcam_name == "gbox":
				cmd = (";rm /tmp/Addon.tgz;wget -q http://magic-image.com./image-feed/enigma2/softcam/gbox_804_magic_E2.tar.gz -O /tmp/Addon.tgz")
			cmd += (';cd /; tar -xz -f /tmp/Addon.tgz ; rm /tmp/Addon.tgz;rm /usr/sbin/nab_e2_restart.sh; chmod 755 /tmp/magic_e2_installer.sh; /tmp/magic_e2_installer.sh; rm /tmp/magic_e2_installer.sh')
