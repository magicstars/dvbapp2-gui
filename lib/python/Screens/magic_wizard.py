...
config.egwizard = ConfigSubsection()
config.egwizard.enablehbbtv = ConfigYesNo(default=False)
config.egwizard.enable3gmodems = ConfigYesNo(default=False)
config.egwizard.enableDLNAServer = ConfigYesNo(default=False)
config.egwizard.enableDLNABrowser = ConfigYesNo(default=False)
config.egwizard.enableWifiDrivers = ConfigYesNo(default=False)
config.egwizard.dsemudmessages = ConfigYesNo(default=False)
config.egwizard.firmwaremicom = ConfigYesNo(default=False)
...
class magicWizardSetup(ConfigListScreen, Screen):
	...
	self.skinName = ["Setup"]
        ...
        list = []
	list.append(getConfigListEntry(_('Enable HBBTV ?'), config.egwizard.enablehbbtv))
	list.append(getConfigListEntry(_('Enable 3G / 4G Modems support ?'), config.egwizard.enable3gmodems))
	list.append(getConfigListEntry(_('Enable Firmware Update Plugin ?'), config.egwizard.firmwaremicom))
	list.append(getConfigListEntry(_('Enable DLNA Server ?'), config.egwizard.enableDLNAServer))
	list.append(getConfigListEntry(_('Enable DLNA Browser ?'), config.egwizard.enableDLNABrowser))
	list.append(getConfigListEntry(_('Enable WiFi drivers ?'), config.egwizard.enableWifiDrivers))
	list.append(getConfigListEntry(_('Enable DVB-T/S/C USB drivers ?'), config.egwizard.enableDvbtDrivers))
